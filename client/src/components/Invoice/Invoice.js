...
const Invoice = () => {
  ...
  }, [location, dispatch, user.result?._id, user.result.googleId, products]);
  useEffect(() => {
  ...
  const handleChangeProducts = (index, e) => {
    console.log(index, e);
    const values = [...invoiceData.items];
    values[index].itemName = e.name;
    values[index].unitPrice = e.price;
    setInvoiceData({ ...invoiceData, items: values });
  };
