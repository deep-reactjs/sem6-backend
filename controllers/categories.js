...
export const getCategories = async (req, res) => {
    ...
    try {
        ...
        const categories = await CategoryModel.find().sort({ _id: -1 }).limit(LIMIT).skip(startIndex);

        res.json({ data: categories, total: total, currentPage: Number(page), recordsPerPage: LIMIT, numberOfPages: Math.ceil(total / LIMIT)});
    } catch (error) {    
        res.status(404).json({ message: error.message });
    }
}
...
console.log(category)
    const newCategory = new CategoryModel({...category, createdAt: new Date().toISOString() })
...
export const updateCategory = async (req, res) => {
    ...
    const updatedCategory = await CategoryModel.findByIdAndUpdate(_id, {...category, _id}, { new: true})
